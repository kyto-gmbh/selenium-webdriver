...
// see https://github.com/detro/ghostdriver/blob/master/binding/java/src/main/java/org/openqa/selenium/phantomjs/PhantomJSDriver.java
// for the Java implementation
webdriver.WebDriver.prototype.executePhantomScript = function(script, var_args) {
  if (goog.isFunction(script)) {
  ...
  return this.schedule(
    new webdriver.Command(webdriver.CommandName.EXECUTE_PHANTOM_SCRIPT).
      setParameter('script', script).
      setParameter('args', goog.array.slice(arguments, 1)),
    'GhostDriver.executePhantomScript()');
};
